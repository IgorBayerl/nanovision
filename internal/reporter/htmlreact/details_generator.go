	...
	for _, method := range fileNode.Methods {
		...
		if fileNode.Diff != nil {
			...
			if newLinesTotal > 0 {
				...
				md.Metrics["d_patch_line_coverage"] = methodMetric{
		...
		if method.CyclomaticComplexity != nil {
			...
			md.Metrics["f_cyclomatic_complexity"] = methodMetric{
