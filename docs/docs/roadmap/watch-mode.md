...
*   **Configuration File Watching**: Extend the watch functionality to monitor a project-specific configuration file (e.g., `.nanovision.yml`). If the configuration changes (e.g., a filter is updated), the tool would automatically reload the settings and regenerate the report.
