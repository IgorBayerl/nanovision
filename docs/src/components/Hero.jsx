...
const Hero = () => {
    ...
    const patchText = "It takes your coverage files (LCOV, Cobertura, GoCover, GCov), crunches the numbers, do some static analysis, and spits out many report formats. Simple as that.";
    ...
    // State: { [index]: 'covered' | 'uncovered' }
    const [wordStates, setWordStates] = useState({});
    const resetTimerRef = React.useRef(null);

    // --- Interaction Handler ---
    const handleHover = (globalIndex) => {
        // Clear existing timer on interaction
        ...
        setWordStates((prev) => {
            ...
            if (!currentState) {
                newState = 'covered'; // First pass: Green
            } else if (currentState === 'covered') {
                newState = 'uncovered'; // Second pass: Red
            } else {
                newState = 'covered'; // Toggle back to Green
            }

            return { ...prev, [globalIndex]: newState };
        });

        // Set new timer to reset after 5 seconds of inactivity
    ...
    // Only 'covered' (green) counts towards the score
    const coveredCount = Object.values(wordStates).filter(s => s === 'covered').length;
    const lineCoverage = totalWords > 0 ? Math.min(100, Math.round((coveredCount / totalWords) * 100)) : 0;

    const isTitleFullyCovered = titleWords.every((_, i) => wordStates[i] === 'covered');
    const isDescFullyCovered = patchWords.every((_, i) => wordStates[titleWordCount + i] === 'covered');
    ...
    const patchCoverage = patchWordCount > 0 ? Math.min(100, Math.round((patchWordsCovered / patchWordCount) * 100)) : 0;
    ...
    const renderInteractiveWords = (words, offset, isPatch = false) => {
        ...
        <section className="relative pt-32 pb-24 overflow-hidden">
            <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
                ...
                <div className="text-center lg:text-left space-y-8">
                    <a href="https://github.com/IgorBayerl/nanovision/releases" target="_blank" className="inline-flex items-center gap-3 px-3 py-1 rounded-full border border-primary/20 bg-primary/5 text-primary text-xs font-mono mb-4 hover:bg-primary/10">
                        <span className={`relative flex h-2 w-2`}>
                            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${lineCoverage === 100 ? 'bg-green-400' : 'bg-primary'}`}></span>
                            <span className={`relative inline-flex rounded-full h-2 w-2 ${lineCoverage === 100 ? 'bg-green-500' : 'bg-primary'}`}></span>
                        </span>
                    ...
                    <div className="text-lg text-foreground/90 leading-relaxed max-w-xl mx-auto lg:mx-0 font-normal select-none relative">
                        <div className="absolute left-[-18px] top-0 text-primary font-mono opacity-50 select-none pointer-events-none">+</div>
                        <p className="mb-2 flex flex-wrap gap-x-1">
                            {renderInteractiveWords(patchWords, titleWordCount, true)}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start pt-4">
                        ...
                        {/* <a href="/getting-started" className="w-full sm:w-auto border border-input bg-background text-foreground px-8 py-4 rounded-lg font-medium flex items-center justify-center gap-2 hover:bg-secondary">
                            <FileText size={20} />
                            Documentation
                        </a> */}
                    </div>
                    ...
                    <div className={`relative z-10 space-y-4 animate-float`}>
                        <div className="bg-card border border-border rounded-2xl p-6 shadow-2xl">
                            <div className="flex items-center justify-between border-b border-border pb-4 mb-6">
                                ...
                                <div className="text-xs font-mono text-muted-foreground">coverage-report/index.html</div>
                            </div>
                        ...
                        <div className="absolute -right-8 -bottom-12 bg-popover border border-border p-4 rounded-lg shadow-xl hidden lg:block animate-pulse">
                            <pre className="text-xs font-mono text-muted-foreground">
                                <span className="text-primary">nanovision</span> \<br />
                                &nbsp;&nbsp;-report="coverage.out"
                            </pre>
                        </div>
                    </div>
